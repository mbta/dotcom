defmodule SiteWeb.TripPlanV1ControllerTest do
  use SiteWeb.ConnCase
  alias GoogleMaps.MapData.{Marker}
  alias Schedules.Repo
  alias Site.TripPlan.MapV1
  alias Site.TripPlan.Query
  alias SiteWeb.TripPlanV1Controller, as: TripPlanController
  alias SiteWeb.TripPlanV1View
  alias TripPlan.{Api.MockPlanner, NamedPosition}
  import Phoenix.HTML, only: [html_escape: 1, safe_to_string: 1]
  doctest SiteWeb.TripPlanV1Controller
  ...
  describe "index without params" do
    test "renders index.html", %{conn: conn} do
      conn = get(conn, trip_plan_v1_path(conn, :index))
  ...
  describe "index with params" do
    test "renders the query plan", %{conn: conn} do
      conn = get(conn, trip_plan_v1_path(conn, :index, @good_params))
    ...
    test "uses current location to render a query plan", %{conn: conn} do
      ...
      conn = get(conn, trip_plan_v1_path(conn, :index, params))
    ...
    test "renders a geocoding error", %{conn: conn} do
      conn = get(conn, trip_plan_v1_path(conn, :index, @bad_params))
      response = html_response(conn, 200)
      assert response =~ "Trip Planner"
    ...
    test "renders a prereq error with the initial map", %{conn: conn} do
      conn = get(conn, trip_plan_v1_path(conn, :index, plan: %{"from" => "", "to" => ""}))
      response = html_response(conn, 200)
    ...
    test "bad date input: too far in future", %{conn: conn} do
      end_date = Timex.shift(Repo.end_of_rating(), days: 1)
      ...
      expected =
        [:too_future]
        |> TripPlanV1View.plan_error_description()
        |> IO.iodata_to_binary()
    ...
    test "destination address has a checkmark in its stop bubble", %{conn: conn} do
      ...
      morning_conn = get(conn, trip_plan_v1_path(conn, :index, params))
  ...
  describe "add_initial_map_markers/2" do
    test "adds a map marker if address has a lat/lng", %{conn: conn} do
      ...
      initial_data = MapV1.initial_map_data()
