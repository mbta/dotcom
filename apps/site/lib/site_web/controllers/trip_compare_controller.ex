defmodule SiteWeb.TripCompareController do
  @moduledoc """
  ...
  defp render_plan(conn, plan) do
    [current_results, new_results] =
      Util.async_with_timeout(
        [
          fn -> get_itineraries(conn, plan, :root_url, "CURRENT") end,
          fn -> get_itineraries(conn, plan, :root_url_compare, "NEW") end
        ],
        {:error, :timeout},
        __MODULE__
      )
  ...
  defp get_itineraries(conn, plan, url_key, source) do
    plan = Map.put(plan, "root_url", Util.config(:trip_plan, OpenTripPlanner, url_key))
  ...
  defp breadcrumbs(conn, _) do
    assign(conn, :breadcrumbs, [Breadcrumb.build("Draft Redesigned Bus Network Compare")])
  end
