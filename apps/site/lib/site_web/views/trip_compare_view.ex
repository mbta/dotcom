defmodule SiteWeb.TripCompareView do
  @moduledoc "Contains the logic to compare results from two Trip Planners"
  use SiteWeb, :view
  ...
  def itinerary_html(itineraries, %{conn: conn, expanded: expanded}) do
    old_count = Enum.count(itineraries, &(&1.source == "CURRENT"))
    ...
    for {i, routes, map, links, itinerary_row_list, index} <-
      ...
      tab_html =
        ...
        |> render_to_string(
          itinerary: i,
          index: if(index > old_count, do: index - old_count, else: index),
          routes: routes,
