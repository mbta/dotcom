...
function togglePslFilter(pslType) {
  const locationCards = [ ...document.querySelectorAll(".c-sales-locations__card") ]
...
function pslTypeMatch(cardType, buttonType) {
  return buttonType.toLowerCase().includes("all") || cardType.toLowerCase() === buttonType.toLowerCase() || cardType.toLowerCase().includes("both");
}

export function setupPslFilterButtons() {
  const pslButtons = [
    ...document.querySelectorAll(".psl-type-button")
  ];
