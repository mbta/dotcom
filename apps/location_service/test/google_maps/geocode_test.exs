defmodule LocationService.GeocodeTest do
  ...
  describe "geocode/1" do
    ...
    test "returns :ok for one valid responses" do
      ...
      assert %LocationService.Geocode.Address{} = result
    end
    ...
    test "returns :ok for multiple matches" do
      ...
      assert %LocationService.Geocode.Address{} = result1
      assert %LocationService.Geocode.Address{} = result2
    end
    ...
    test "uses cache" do
      ...
      assert %LocationService.Geocode.Address{} = cache_miss_result

      cache_hit = geocode(address)
      assert {:ok, [cache_hit_result]} = cache_hit
      assert %LocationService.Geocode.Address{} = cache_hit_result
    end
  end
  ...
  describe "calculate_position/2" do
    test "it calculates search position and address" do
      ...
      geocode_fn = fn _address ->
        {:ok,
         [%LocationService.Geocode.Address{formatted: "address", latitude: 42.0, longitude: -70.1}]}
      end
