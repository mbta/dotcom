defmodule DotcomWeb.Components do
  ...
  def count(%{count: 0} = assigns) do
    ~H"""
    <.badge class="bg-charcoal-80 text-black !min-w-6 md:!min-w-7 py-1.25 text-sm md:text-base" variant="square">
      0
    </.badge>
    """
  end
  ...
  def count(%{count: _} = assigns) do
    ~H"""
    <.badge class="bg-cobalt-30 text-white !min-w-6 md:!min-w-7 py-1.25 text-sm md:text-base" variant="square">
      {@count}
    </.badge>
    """
  end
