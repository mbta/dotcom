defmodule DotcomWeb.Components.TransitIcons do
  ...
  def accessibility(assigns) do
    ~H"""
    <.icon type="icon-svg" name="icon-accessible-small" class="h-6 w-6" aria-label={~t"Accessible"} />
    """
  end
  ...
  def parking(assigns) do
    ...
    <div class="bg-gray-light text-white h-6 px-1.5 w-6 rounded-sm inline-flex items-center font-bold justify-center">
      <span class="notranslate" aria-label={~t"Parking"}>P</span>
    </div>
    """
  end
  ...
  def zone(assigns) when not is_nil(assigns.stop.zone) do
    ...
    <div class="border-[1px] border-commuter-rail text-commuter-rail max-h-6 rounded-sm px-1.5 font-bold inline-flex justify-center text-nowrap">
      {gettext("Zone %{zone}", zone: @stop.zone)}
    </div>
    """
  end
