defmodule Dotcom.TripPlanner.FarePasses do
  ...
  defp highest_month_pass(
         %Leg{
           mode: %TransitDetail{route: route, trip_id: trip_id},
           from: %NamedPosition{stop: origin},
           to: %NamedPosition{stop: destination}
         } = leg
       ) do
    if Leg.fare_complete_transit_leg?(leg) do
      Month.base_pass(route.id, trip_id, origin.id, destination.id)
    else
      nil
    end
  end
  ...
  defp lowest_month_pass(
    ...
    if Leg.fare_complete_transit_leg?(leg) do
      Month.recommended_pass(route.id, trip_id, origin.id, destination.id)
    else
      nil
    end
  end
  ...
  defp reduced_pass(
    ...
    if Leg.fare_complete_transit_leg?(leg) do
      Month.reduced_pass(route.id, trip_id, origin.id, destination.id)
    else
      nil
    end
  end
